import { apiBaseUrl, Category, Location, UnauthorizedError } from "./Shared";
...
export async function createTravelGuide(
    ...
    const response = await fetch(apiBaseUrl + "/travel-guides", {
        method: "POST",
        headers: new Headers({
...
export async function updateTravelGuide(
    ...
    const response = await fetch(apiBaseUrl + "/travel-guides/" + id, {
        method: "PUT",
        headers: new Headers({
            "content-type": "application/json",
            "x-tg-secret": secret,
